...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("wanted *object.Function, but got %T (+%v)", evaluated, evaluated)
	}

	if len(fn.Parameters) != 1 {
		t.Fatalf("fn has wrong parameters number want %d, but got %d", 1, len(fn.Parameters))
	}

	if fn.Parameters[0].String() != "x" {
		t.Fatalf("fn has wrong parameter args want %s, but got %s", "x", fn.Parameters[0].String())
	}

	expectedBody := "(x + 2)"

	if fn.Body.String() != expectedBody {
		t.Fatalf("fn body evaluated error, want %q, but got %q", expectedBody, fn.Body.String())
	}
}
...
func TestBuiltinFunction(t *testing.T) {
	...
	for _, test := range tests {
		...
		case string:
			errObj, ok := evaluated.(*object.Error)
			if !ok {
				t.Errorf("object is not an Error object, got %T (%+v)", evaluated, evaluated)
				continue
			}

			if errObj.Message != expected {
				t.Errorf("wrong error message, expected %q, but got %q", expected, errObj.Message)
			}
		}
	}
}
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Errorf("object is not Array Literal, got %T (%+v)", evaluated, evaluated)
	}
...
func testNullObject(t *testing.T, obj object.Object) bool {
	if obj != NULL {
		t.Errorf("expect obj is null, got %T (%+v)", obj, obj)
		return false
	}
