...
// prefix func takes no args cuz no need to parsed the left portion of prefix
// infix func takes args of left side expression to continuing parsing the remaining expression
type (
...
func (p *Parser) parseGroupedExpression() ast.Expression {
	...
	// when current parsed token's next is right parenthesis
	// parseExpression gonna return the left portion of parenthesis
	// thus it is valid to parsed with parenthesis
	if !p.expectedPeek(token.RPAREN) {
		return nil
	}
...
func (p *Parser) parseFunctionParameters() []*ast.Identifier {
	...
	// check scenario of fn() -> no parameter
...
// parsing call function like fn(3 + 1, 5 * 5) etc...
// treat every argument as a Expression and recursively call
// parseExpression to deal with parsing function calls
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		msg := fmt.Sprintf("error parsing token literal %q to integer", p.curToken.Literal)
...
// check do we have a prefix function in prefix position
func (p *Parser) parseExpression(precedence int) ast.Expression {
	// defer untrace(trace("parseExpression"))
	// prefixParseFn value gives the function to parse prefix and return the corresponding Expression
...
func (p *Parser) parseHashLiteral() ast.Expression {
	...
	for !p.peekTokenIs(token.RBRACE) {
		...
		// check if peek token needed to skip
...
// check expected Token for appending next token
...
// used by parsing unknown token
func (p *Parser) noPrefixParseError(t token.TokenType) {
	msg := fmt.Sprintf("no valid prefix parse function for %s", t)
	p.errors = append(p.errors, msg)
}
